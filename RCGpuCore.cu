...
// x[3] = bits 192-255 (most significant)
// x[2] = bits 128-191
// x[1] = bits 64-127
// x[0] = bits 0-63 (least significant)
__device__ __forceinline__ int clz256(const u64* x)
{
	// Check from most significant to least significant
	if (x[3] != 0) return __clzll(x[3]);                    // Leading zeros in top 64 bits
	if (x[2] != 0) return 64 + __clzll(x[2]);               // 64 + leading zeros in bits 128-191
	if (x[1] != 0) return 128 + __clzll(x[1]);              // 128 + leading zeros in bits 64-127
	if (x[0] != 0) return 192 + __clzll(x[0]);              // 192 + leading zeros in bottom 64 bits
...
__global__ void KernelA(const TKparams Kparams)
{
	...
	// Prefetch jump tables to shared memory - unrolled for better ILP
	int i = THREAD_X;
	while (i < JMP_CNT)
    {
		// Use __ldg for read-only cache (texture cache)
    ...
    for (int step_ind = 0; step_ind < STEP_CNT; step_ind++)
    {
        ...
        for (int group=PNT_GROUP_CNT-1; group>=0; group--) {
            ...
            MulModP(Xg, Xg, invZ2);
            // check DP - with proper bounds checking to prevent illegal memory access
            if ((Xg[3] & dp_mask64) == 0) {
